  ...
  <section class="splash-block splash-block--dark">
    <div class="container">
      <header class="splash-summary__header">
        <h2>RSS Re-Feeder</h2>
        <input v-model="rssUrl" placeholder="Enter RSS Re-Feed URL" />
        <button @click="importRefeed">Import Re-Feed</button>
      </header>
    </div>
  </section>
</template>
...
export default {
  ...
  setup() {
    ...
    const importRefeed = async () => {
      if (!rssUrl.value) {
        alert("Please enter a valid RSS re-feed URL.");
        return;
      }

      try {
        await addDoc(collection(db, "refeeders"), {
          url: rssUrl.value,
          siteID: otSiteID,
          createdAt: serverTimestamp(),
        });

        console.log("Re-Feed request submitted to Cloud Function.");
        rssUrl.value = "";
      } catch (err) {
        console.error("Error submitting re-feed request:", err);
      }
    };
